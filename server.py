...
# Render и Railway автоматически назначают порт через переменную окружения PORT
...
class Room:
    def __init__(self, code, host_client, host_name):
        ...
        self.category = None
        self.started = False
        self.ready_players = set()  # Сокеты игроков, подтвердивших прочтение роли
...
async def main():
    print(f"Сервер Spyfall запущен на порту {PORT}...")
    # 0.0.0.0 позволяет принимать подключения извне (обязательно для хостингов)
